 ...
 *   get:
 *     tags: [Developer]
 *     summary: List a Thread's workspace files (the agent's own virtual filesystem)
 *     description: >
 *       The same `{ files }` data already embedded in `GET .../messages`'s
 *       `state.files` — a lighter, dedicated read for a caller that only
 *       wants the files.
 *     security: [{ projectCredential: [] }]
 *     parameters:
 *       - name: threadId
 ...
 *         schema: { type: string }
 *     responses:
 *       200: { description: "{ [path]: { content, size, created_at, modified_at } }" }
 *       404: { description: Thread not found or unauthorized }
 */
router.get('/:threadId/files', developerThreadController.listWorkspaceFiles);
 ...
 *   get:
 *     tags: [Developer]
 *     summary: Get one workspace file's content
 *     security: [{ projectCredential: [] }]
 *     parameters:
 *       - name: threadId
 *         in: path
 ...
 *         schema: { type: string }
 *     responses:
 *       200: { description: "{ content, size, created_at, modified_at }" }
 *       404: { description: Thread or file not found, or unauthorized }
 *   put:
 *     tags: [Developer]
 *     summary: Create or overwrite one workspace file
 *     description: >
 *       Writes directly into the thread's live agent checkpoint via
 *       LangGraph's updateState — the same sanctioned pattern the voice
 *       feature already uses for the messages channel. KNOWN LIMITATION —
 *       no lock exists against a concurrent live run on this same thread;
 *       a write here while the agent is actively mid-run could race with
 *       its own file writes.
 *     security: [{ projectCredential: [] }]
 *     parameters:
 *       - name: threadId
 ...
 *     responses:
 *       200: { description: "{ content, size, created_at, modified_at }" }
 *       400: { description: "Missing path/content, path targets /skills/..., or the Thread has no Agent" }
 *       404: { description: Thread not found or unauthorized }
 *   delete:
 *     tags: [Developer]
 *     summary: Delete one workspace file
 *     description: Same live-checkpoint write and concurrency caveat as PUT.
 *     security: [{ projectCredential: [] }]
 *     parameters:
 *       - name: threadId
 *         in: path
 ...
 *       404: { description: Thread or file not found, or unauthorized }
 */
router.get('/:threadId/file', developerThreadController.getWorkspaceFile);
router.put('/:threadId/file', developerThreadController.writeWorkspaceFile);
router.delete('/:threadId/file', developerThreadController.deleteWorkspaceFile);
