 ...
 */
export const listProvidersTool = (userId) => new DynamicStructuredTool({
  name: 'list_my_providers',
  description: 'Lists all LLM providers (base URLs, default models) configured by the user. Use this to help the user choose a model for their agent. Sensitive keys are NOT included.',
  schema: z.object({}),
  func: async () => {
 ...
 */
export const upsertAgentTool = (userId) => new DynamicStructuredTool({
  name: 'upsert_agent',
  description: 'Creates or updates an agent configuration. Handles basic info, instructions, and runtime settings.',
  schema: z.object({
    agentId: z.string().optional().describe('ID of the agent to update. Leave empty to create a new one.'),
    ...
    systemPrompt: z.string().optional().describe('The primary instructions defining the agent behavior'),
 ...
 */
export const manageSkillTool = (userId) => new DynamicStructuredTool({
  name: 'manage_skill',
  description: 'Creates, updates, or lists skills owned by the user. Skills are standalone logic blocks that can be attached to agents.',
  schema: z.object({
    ...
    try {
      switch (input.action) {
        case 'list':
          ...
          return JSON.stringify(skills.map(s => ({ id: s._id, name: s.name, description: s.description })));
        case 'create':
