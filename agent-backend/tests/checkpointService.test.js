...
// getMessages builds the agent to inspect .tasks for pending interrupts —
// mocked so that path resolves harmlessly (no interrupts) rather than
// hitting the real, unmocked factory.
const mockGetState = jest.fn();
jest.unstable_mockModule('../src/modules/agents/agent.factory.js', () => ({
  default: {
    buildAgent: jest.fn(async () => ({
      agentInstance: { getState: mockGetState },
      llm: null,
    })),
  },
}));
