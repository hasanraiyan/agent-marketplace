...
1. **Maintain Current Structure**: Keep files sorted by technical layer and only refactor logic within existing boundaries.
   * *Rejected*: Fails to resolve the underlying friction of feature maintenance and does not provide clean architectural boundaries.
2. **NestJS Migration**: Complete transition to NestJS.
   * *Rejected*: Explicitly prohibited by absolute constraints. Adding NestJS introduces high compile-time overhead and alters developer workflows.
