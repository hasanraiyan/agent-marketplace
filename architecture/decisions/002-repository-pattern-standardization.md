...
1. **Allow Direct Model Access**: Let services and controllers query models directly.
   * *Rejected*: Promotes code duplication (e.g., repeating the same `.find()` parameters) and ties the business logic layer to the MongoDB ODM.
2. **Generic Base Repository**: Introduce an abstract generic base repository pattern.
   * *Rejected*: Introduces excessive boilerplate and NestJS-like over-engineering without solving a specific problem in a small JS project.
