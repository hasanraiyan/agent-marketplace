...
`@personaai/devtools` is the devtools for the Persona SDKs. One package, three subpaths (`core` / `react` / `nextjs`), keeps the `sdk/devtools/{core,react,nextjs}` layout you approved.
...
<Note>
  **Dev-only, zero runtime changes.** No new route is required. The panel is a pure client component that reads the hooks you already use. It never bundles into production unless you import it.
</Note>
...
```tsx
<PersonaDevtools baseUrl="/api/persona" clientState={{ threads, messages, files, todos }} />
```
...
This package has no dependency on `@personaai/runtime` (`sdk/devtools/package.json` only `logger` + optional `react`). It composes the runtime via the existing `PersonaContext` and optional `fetch(__persona/devtools)` — zero lines changed in `sdk/runtime`.
