---
title: Quickstart
description: Mount the Persona runtime as an Express Router with @personaai/adapters/express — streaming chat, threads, files, and memory in a few lines, behind your own auth.
---

<Note>
  **Package has moved:** `npm install @personaai/adapters` then `import from '@personaai/adapters/express'`. The old `@personaai/express` still works as a deprecated shim re-exporting from the unified package and will be removed in 1.0.
</Note>

`@personaai/adapters/express` exposes the [Persona runtime](/guides/runtime/quickstart) as an Express
