---
title: Quickstart
description: Add AI to your Next.js app with @personaai/adapters/nextjs — one catch-all route mounts the runtime, one provider wires the hooks, behind your own auth.
---

<Note>
  **Package has moved:** `npm install @personaai/adapters` then `import from '@personaai/adapters/nextjs'` / `'@personaai/adapters/nextjs/server'`. The old `@personaai/nextjs` still works as a deprecated shim re-exporting from the unified package and will be removed in 1.0.
</Note>

`@personaai/adapters/nextjs` is **the one package a Next.js app installs**. One App Router catch-all route
...
<Warning>
  **The credential is server-side only.** It lives behind `@personaai/adapters/nextjs/server`, which the
...
imports from `@personaai/adapters/nextjs`. They are the [React SDK](/guides/react/quickstart)'s hooks
...
| --- | --- | --- |
| `@personaai/adapters/nextjs` | Everything from `@personaai/react` — `PersonaProvider` and every hook | Client (`'use client'` boundary) |
| `@personaai/adapters/nextjs/server` | `createPersonaHandler`, `toNextRouteHandlers`, `createRuntime`, and the runtime types | Server only |
...
the runtime — see the [Runtime Hooks reference](/guides/runtime/hooks):
...
- **Real-time voice**, powered by Gemini Live — `useVoice()` from the same import, `POST
  /api/persona/voice/sessions` from the same catch-all route, nothing extra to mount. See the
  [voice guide](/guides/nextjs/voice).
...
- [Voice](/guides/nextjs/voice) — real-time voice calls with `useVoice()`, end to end.
