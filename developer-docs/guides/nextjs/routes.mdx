---
title: Routes
description: Every route a @personaai/adapters/nextjs mount exposes, how the adapter translates a Next.js Request, and which SDK call each route proxies to.
---
...
| `POST` | `/voice/sessions` | Mints a real-time voice call ticket — `201`. Unlike every other row here, the actual call does not go through this route or this app at all; see the [voice guide](/guides/nextjs/voice). |
...
import { createRuntime, toNextRouteHandlers } from '@personaai/adapters/nextjs/server';
...
`restToolsManifest` to `createPersonaHandler`/`createRuntime`, it's never gated behind
`capabilities`, and it never calls `PersonaClient` — it just serves data you configured, so
`resolveUserFrom` is never invoked for it either. See [REST Tool
