...
- **Always on** — things an end user does in their own chat session: send messages, manage their
  own conversations/files/memory, connect their own MCP account, start a real-time voice call.
  Scoped entirely to whichever user `resolveUser` returns for that request.
...
`/memory`, `/voice`), not necessarily your team's internal tooling.
...
## Not a capability: `restToolsManifest`

`restToolsManifest` (see [Routes](/guides/runtime/routes#static-manifest-route--a-third-tier-neither-always-on-nor-capability-gated))
looks similar to a capability flag but isn't one — it's not gated behind `capabilities`, doesn't
default off for security reasons, and doesn't proxy anything through `PersonaClient`. It's a plain
option that makes `createRuntime()` serve a static list of your own REST tool definitions at
`GET {mountPath}/rest-tools/manifest`, secured by its own `authToken` bearer check instead of
`resolveUser`. Enable it any time you want a Persona REST Tool Source to discover tools from this
host — there's no admin-surface exposure risk to weigh the way there is with `providers: true`.
