...
There's no published `@personaai/node` adapter yet, so the package ships a small, tested bridge in
`examples/` for running the runtime directly against Node's `http` module. It is **not a stable
public API** — just enough to demo/smoke-test the runtime end-to-end until a real adapter ships.
It also parses multipart file uploads via Node's native `Request`/`FormData` (undici), no extra
dependency:
