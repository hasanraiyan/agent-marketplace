...
| `GET` | `/mcps/:id/resource?uri=` | `client.mcps.readResource(id, uri)` — reads an MCP Ext App UI resource (always-on since `0.9.4`) |
| `POST` | `/mcps/:id/call-tool` | `client.mcps.callTool(id, name, arguments)` — direct widget tool invocation (always-on since `0.9.4`) |
...
## Static manifest route — a third tier, neither always-on nor capability-gated

| Method | Path | Auth | Proxies to |
| --- | --- | --- | --- |
| `GET` | `/rest-tools/manifest` | Its own `Authorization: Bearer <authToken>` check, `requiresAuth: false` | Nothing — serves `restToolsManifest.tools` verbatim as `{ tools: [...] }` |

This route only exists at all when you pass `restToolsManifest` to `createRuntime()`. It's
deliberately **not** a `RuntimeCapabilities` flag: every other route in this file is either scoped
to whichever user `resolveUser` resolves, or proxies a Project-admin action through
`PersonaClient`. This one does neither — it's pure host-side static data (a list of REST tool
definitions you built, typically with `defineRestTool()`) that Persona itself pulls from, the
same way it discovers an MCP server's tools. `resolveUser` is never called for it.
...
const getProfile = defineRestTool({
  ...
  method: 'GET',
  url: (t) => `https://api.yourapp.com/users/${t.arg('userId')}`,
...
const runtime = createRuntime({
  ...
  restToolsManifest: {
    tools: [getProfile],
    // Checked against the request's `Authorization: Bearer <token>` header.
    // Must match the plaintext value of the Secret you pick on the
    // Persona-side REST Tool Source. Omit only for local/dev testing — it
...
`https://your-app.com{mountPath}/rest-tools/manifest`, Auth = API Key → pick or create a Secret
(the same Secrets tab REST API Tools use) whose value is `PERSONA_TOOLS_SECRET` →
**Test Connection** discovers `getProfile`. Attach the *source* to an
Agent (the same attachment model as an MCP connector) — Persona re-fetches this route live on
every Agent turn; nothing about the tool list is cached in Persona's database. See [REST Tool
Sources](/guides/rest-tools#code-first-rest-tool-sources) for the full walkthrough and
[`defineRestTool`](/guides/sdk/resources/rest-tools) for the builder's API.

## RCP manifest route — the same third tier, for the open RCP protocol

| Method | Path | Auth | Proxies to |
| --- | --- | --- | --- |
| `GET` | `/rcp/manifest` | Its own `Authorization: Bearer <authToken>` check, `requiresAuth: false` | Nothing — serves a conformant `{ rcpVersion, auth, tools }` manifest |

Independent of `restToolsManifest` above — RCP (REST Connector Protocol) is a separate, standalone
open protocol (npm `rcp-sdk`), not a Persona-specific format. This route only exists when you pass
`rcpManifest` to `createRuntime()`, same non-capability-gated reasoning as `restToolsManifest`:
pure host-side static data, never proxied through `PersonaClient`.
...
const runtime = createRuntime({
  ...
  rcpManifest: {
    tools: [getWeather],
    // Checked against the request's `Authorization: Bearer <token>` header,
    // and reflected in the served manifest's own `auth` field. Omit only for
    // local/dev testing — it leaves the route open (`auth: { type: 'none' }`).
    authToken: process.env.PERSONA_TOOLS_SECRET!,
  },
});
```

Then, in the Persona dashboard: **Add RCP Source** → URL = `https://your-app.com{mountPath}/rcp/manifest`,
Auth = Header → pick or create a Secret whose value is `PERSONA_TOOLS_SECRET` → **Test Connection**
discovers `get_weather`. Attach the *source* to an Agent — Persona re-fetches this route live on
every Agent turn, via `rcp-sdk`'s own client, exactly like it does for an MCP server or a REST Tool
Source.
