...
- **SDK version:** `0.2.x` (currently `0.2.3`). The docs in this section track that release.
...
- **Parity with the Node SDK:** `personaai` mirrors `@personaai/sdk`'s resource surface. Known
  differences as of this release:
  - No `Memory` or `Stores` resources, and no `Architect` chat client (the Node SDK has these; the
