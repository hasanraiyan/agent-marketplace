...
skill = persona.skills.create(
    ...
    idempotency_key="a-key-you-generate-once-per-logical-create",  # e.g. str(uuid.uuid4())
)
```
...
| `.audit_logs`             | `/api/v1/developer/audit-logs`                                     | Control-plane only — no per-user audit trail.                          |
...
- **A real pagination envelope, except on Providers.** Every `list()`-style method
  (`skills.list()`, `agents.list()`, `knowledge.list()`, `mcps.list()`, `threads.list()`,
  `files.list()`) returns `PaginatedResult[T]` — `{"items": list[T], "pagination": {"total",
"page", "limit", "pages"}}` — not a bare list. Check `pagination["pages"]` (or `["total"]`) to
  know whether there's more, rather than inferring it from `len(items)` vs. `limit`.
  `providers.list()` is the one exception: a plain bare list, no params at all — Providers have no
...
usage = persona.providers.get_usage(provider["id"])
persona.providers.bulk_delete([provider["id"]])
```
...
`get_usage()` returns `ResourceUsage` — `{"agentCount": int, "agents": [{"_id", "name"}, ...]}` —
the Agents currently referencing this Provider. `agents` is a preview capped at 20 entries;
...
`bulk_delete(ids)` returns `BulkDeleteResult` — `{"deleted": list[str], "failed":
[{"id": str, "reason": str}, ...]}` — a best-effort batch delete, available on every resource
below. One blocked/not-found id in the batch doesn't abort the rest; check `"failed"` for anything
that didn't go through. Up to 100 ids per call — a request over that limit is rejected with a 400
...
usage = persona.skills.get_usage(skill["_id"])  # check before delete()
persona.skills.delete(skill["_id"])
persona.skills.bulk_delete([skill["_id"]])
```
...
result = persona.agents.list({"category": "productivity", "search": "career"})
discovered = result["items"]
# result["pagination"]: {"total", "page", "limit", "pages"} — check "pages" instead of len() vs. limit
...
persona.agents.bulk_delete([agent["_id"]])
```
...
usage = persona.knowledge.get_usage(kb["_id"])  # check before delete()
persona.knowledge.bulk_delete([kb["_id"]])

bases = persona.knowledge.list({"scope": "mine"})["items"]
```
...
usage = persona.mcps.get_usage(mcp["_id"])  # check before delete()
persona.mcps.bulk_delete([mcp["_id"]])

connectors = persona.mcps.list({"scope": "mine"})["items"]
```
...
user_persona.threads.bulk_delete([thread["_id"]])
```
...
`update()` is the general form — `{title, isArchived}`, both optional, send just the one you want
to change. `update_title(thread_id, title)` is a convenience wrapper over `update()` for the
...
result = user_persona.files.list({"page": 1, "limit": 20})
mine = result["items"]
...
user_persona.files.bulk_delete([file["id"]])
```
...
constructed with `external_user_id` gets an empty `PaginatedResult` back, not an error, matching
...
```python
result = persona.audit_logs.list(
    {"eventType": "credential.created", "page": 1, "limit": 20}  # eventType is an optional filter
)
events = result["items"]
```
...
`ListAuditLogsParams`: `page`, `limit`, `eventType` (optional exact-match filter), all optional.
Each `AuditLogEntry`: `{"eventType", "timestamp", "actorContextType", "actorIdentity",
"targetDomain", "targetResourceId", "metadata"}` — `actorIdentity`/`targetResourceId` are
`str | None`.
...
<Warning>
  ...
  id) raises `PersonaApiError` with `status_code: 404`. It does **not** silently fall back to a
