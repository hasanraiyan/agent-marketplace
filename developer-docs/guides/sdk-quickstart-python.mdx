...
<Note>
  **Public PyPI package coming soon.** Once published, install with `pip install
  persona-agent-sdk` — note the PyPI name is `persona-agent-sdk` (`personaai`
  was already too close to an unrelated existing PyPI project's name), but it's
  still imported as `personaai` in code: `from personaai import PersonaClient`.
  Until published, build it locally from source:
</Note>
