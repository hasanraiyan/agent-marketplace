...
- **Bare arrays, not a pagination envelope.** Every `list()`/`discover()`-style method
  ...
  infer whether there's more. `providers.list()` is the same bare-array shape but takes no params
  at all — Providers have no discovery concept, so there's nothing to page or filter by.
