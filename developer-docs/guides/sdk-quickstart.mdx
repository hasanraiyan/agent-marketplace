...
Every resource's `create()` (and `files.upload()`) accepts an optional trailing
`idempotencyKey` argument, sent as the `Idempotency-Key` request header:
...
dropped connection — where you can't tell whether the original request actually reached the
server. Retrying with the **same** key returns the exact original response instead of creating a
second resource; retrying with a **different** (or omitted) key creates a new one as normal. Keys
are scoped per `(domain, credential, external user if asserted)` and expire after 24 hours. This is
fully opt-in — omitting the argument is identical to every version before this feature, no
behavior change.
...
| `.memory`                 | `/api/v1/developer/memory`                                         | Requires `externalUserId` — memory always belongs to one end user.   |
| `.stores`                 | `/api/v1/developer/stores`                                         | Project — config CRUD needs no `externalUserId`; file CRUD needs one only when the target Store's own `scope` is `externalUser` (see [Stores](#stores)). |
| `.files`                  | `/api/v1/developer/files`                                          | Requires `externalUserId` — a File always belongs to one end user.   |
| `.auditLogs`              | `/api/v1/developer/audit-logs`                                     | Control-plane only — no per-user audit trail.                        |
...
- **A real pagination envelope, except on Providers and Memory.** Every `list()`/`discover()`-style
  method (`skills.list()`, `agents.list()`, `knowledge.list()`, `mcps.list()`, `threads.list()`,
  `stores.list()`, `files.list()`) returns `PaginatedResult<T>` — `{ items: T[], pagination: {
  total, page, limit, pages } }` — not a bare array. Check `pagination.pages` (or
  `pagination.total`) to know whether there's more, rather than inferring it from `items.length`
  vs. `limit`. Two exceptions: `providers.list()` (a plain bare array, no params at all — Providers
  have no discovery concept, so there's nothing to page or filter by), and `memory.list()` (returns
  `MemoryListResult`, `{ userFiles, agentMemories }` — memory has no pagination concept either,
  it's a full listing every time). `stores.listFiles()` is also unpaginated — a bare `StoreFile[]`.
...
const provider = await persona.providers.create(
  ...
  "optional-idempotency-key", // safe retry after a timeout replays this exact result
);
...
Every method requires the client to have been constructed with `externalUserId` — memory is
always scoped to one end user, same requirement and rejection behavior as Threads. Full CRUD
parity with what an Agent's own `write_file`/`read_file` tool calls can already do to
`/memories/user/` and `/memories/agent/` — useful for admin/support tooling ("what does this
Agent remember about founder X"), a founder-facing "manage what you've shared" settings page, or
seeding known facts before a user's first conversation.

```ts
const { userFiles, agentMemories } = await userClient.memory.list();

const file = await userClient.memory.getFile({ path: "/memories/user/index.md" });
...
// Agent-scoped memory (private to one Agent, not shared across the user's other Agents)
await userClient.memory.writeFile({
  scope: "agent",
  agentId: agent._id,
...
| `memory.getFile(params)`         | `GET /memory/file`          | `MemoryFile`          |
| `memory.writeFile(input)`        | `PUT /memory/file`          | `MemoryFile`          |
| `memory.deleteFile(params)`      | `DELETE /memory/file`       | `void`                |

`MemoryListResult`: `{ userFiles: MemoryFile[], agentMemories: { agentId, agentName, files:
MemoryFile[] }[] }` — user-global files plus one group per Agent that has agent-scoped memory.
`scope` on `getFile`/`writeFile`/`deleteFile` defaults to `'user'`; `'agent'` requires `agentId`.

<Warning>
  Memory written here goes through the exact same namespace a live Agent run uses — it's real,
  live-visible data, not a separate copy. A `writeFile()` call is indistinguishable from the Agent
  having written that file itself via its own `write_file` tool.
</Warning>

### Stores

Config CRUD (`create`/`list`/`get`/`update`/`delete`) works with a bare Project credential — a
Store's config isn't per-founder. File CRUD is the same, *except* when the Store's own `scope` is
`'externalUser'`: only then does this client need `externalUserId` set, or the call throws a
`PersonaApiError` with `statusCode: 400` (`EXTERNAL_USER_REQUIRED`).

A Store is a named, scoped mount point you assign to Agents (`storeMounts` on
`agents.create()`/`agents.update()`) — a filesystem-backed alternative to
[`contextOverride`](#chat-streamed) for reference material too large to comfortably re-send in the
prompt every turn. An Agent reads from it via its own `read_file`/`ls` tools, on demand, mounted at
`/stores/<name>/`.

```ts
// A shared, read-only reference store — safe to mount on Agents serving many different founders,
...
// A per-founder store — one private partition per external user, resolved automatically
// from whichever founder is actually running the Agent (same mechanism as /memories/user/).
...
| `stores.list(params?)`               | `GET /stores`                        | `PaginatedResult<Store>`   |
| `stores.get(storeId)`                | `GET /stores/{id}`                   | `Store`                    |
| `stores.update(storeId, input)`      | `PATCH /stores/{id}`                 | `Store`                    |
| `stores.delete(storeId)`             | `DELETE /stores/{id}`                | `void`                      |
| `stores.listFiles(storeId)`          | `GET /stores/{id}/files`             | `StoreFile[]`               |
| `stores.getFile(storeId, params)`    | `GET /stores/{id}/file`              | `StoreFile`                 |
| `stores.writeFile(storeId, input)`   | `PUT /stores/{id}/file`              | `StoreFile`                 |
| `stores.deleteFile(storeId, params)` | `DELETE /stores/{id}/file`           | `void`                      |

`scope` (`'domain' | 'externalUser'`) is **immutable after creation** — `UpdateStoreInput`
deliberately doesn't include it; create a new Store if you need a different scope.
`writeFile()`/`deleteFile()` are **not** gated by `accessMode` — `readonly` only blocks an Agent's
own tool calls at mount time, this API is how a readonly Store's content actually gets populated
in the first place.
...
constructed with `externalUserId` gets an empty `PaginatedResult` back, not an error, matching
`providers.list()`'s own existence-hiding-style precedent.
...
<Warning>
  ...
  entry here yet. Don't expect to find an `"agent.deleted"` event; it doesn't
  exist as of this release.
</Warning>

```ts
const { items: events, pagination } = await persona.auditLogs.list({
...
| Method                    | Wraps             | Returns                          |
| ------------------------- | ----------------- | -------------------------------- |
| `auditLogs.list(params?)` | `GET /audit-logs` | `PaginatedResult<AuditLogEntry>` |

`ListAuditLogsParams`: `page?`, `limit?`, `eventType?` (optional exact-match filter). Each
`AuditLogEntry`: `{ eventType, timestamp, actorContextType, actorIdentity, targetDomain,
targetResourceId, metadata }` — `actorIdentity`/`targetResourceId` are `string | null`.
...
one-conversation-per-Agent-per-user thread), `resume?` (see below), `contextOverride?` (a string
appended to *this turn's* system prompt only — never persisted to memory, never visible to later
turns; capped at 4000 characters server-side, rejected with a 400 above that rather than silently
truncated — use this for small, ephemeral facts that change turn-to-turn, e.g. a live profile
snapshot; for larger reference material an Agent should look up on demand, use
[Stores](#stores) instead), `signal?` (an `AbortSignal`).
...
<Warning>
  `threadId` must belong to the asserted external user and the `agentId` you're calling with —
  passing one that doesn't (someone else's Thread, a different Agent's Thread, or a nonexistent
  id) throws a `PersonaApiError` with `statusCode: 404`. It does **not** silently fall back to a
...
| `CUSTOM`                                        | Persona-specific side-channel events — subagent traces (`subagent_activity`), MCP App widgets (`mcp_app`), and this SDK's own human-in-the-loop signaling (`hitl_request` / `clarification_request` — see below). |
| `RUN_ERROR`                                     | The run failed genuinely (not an interrupt) — `code`, `message`, `retryable`, `providerName`. See [Structured run errors](#structured-run-errors) below.                              |
...
`GET /api/v1/developer/agui/schema` (not yet wrapped by a typed method on this client — call it via
`persona` directly if you need it, e.g. through the underlying `HttpClient`). Every stream response
also carries the active schema version on the `X-AGUI-Schema-Version` response header. The
TypeScript counterparts (`ClarificationRequestPayload`, `HitlRequestPayload`, `McpAppPayload`,
`SubagentActivityPayload`) are exported from the package root and kept in sync with that document.
...
If a run ends in a genuine failure — not an interrupt — `sendMessage()` sets `result.error` instead
of `result.interrupt`, as a typed `PersonaRunErrorEvent`:
...
`code` is a `RunErrorCode`: `'PROVIDER_AUTH_ERROR' | 'PROVIDER_RATE_LIMIT' | 'TOOL_TIMEOUT' |
'TOOL_ERROR' | 'CONTEXT_LENGTH_EXCEEDED' | 'INTERNAL_ERROR'`.
...
`McpsResource`, `McpOAuthResource`, `ThreadsResource`, `MemoryResource`, `StoresResource`,
`FilesResource`, `ChatClient`), every create/update/list-params/result interface referenced in the
tables above, `PrincipalContext` and its two variants, `AguiEvent`/`EventType`, the `CUSTOM`-event
payload types (`ClarificationRequestPayload`, `HitlRequestPayload`, `McpAppPayload`,
`SubagentActivityPayload`), `PersonaRunErrorEvent`/`RunErrorCode`, and the error classes from
