...
| `sandboxEnabled`      | `boolean`                              | `false`     | Real shell execution in an isolated CodeSandbox VM. Requires a `CSB_API_KEY` Project Secret to exist first — create one via the platform UI or `POST /api/v1/developer/secrets` (no `secrets` SDK resource yet). |
