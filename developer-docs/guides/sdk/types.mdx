...
| Type    | `VoiceSessionInfo`, `VoiceSessionTicket`                                                            |
...
`client.voice.createSession(agentId)` mints a single-use ticket for the voice WebSocket gateway
(real-time audio, powered by Gemini Live). Call it from your own server, then hand the returned
`wsUrl` to your own frontend, which opens `new WebSocket(wsUrl)` directly — this SDK never opens
the WebSocket or touches audio itself, so your Project credential never reaches a browser.
