...
`@personaai/ui` is a set of pre-built React components for [Persona](https://persona.hasanraiyan.me) — full chat widgets and standalone building blocks that compose on top of `@personaai/react` hooks.

<Tip>
  **Looking for zero CSS clashes?** We recommend using the new [Persona Shadcn Component Registry](/guides/ui/shadcn-registry). Install modular source components directly into your codebase with `npx shadcn@latest add https://platform.persona.hasanraiyan.me/r/chat.json` to compile cleanly with your own Tailwind configuration.
</Tip>
