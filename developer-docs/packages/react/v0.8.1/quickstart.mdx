...
**v0.8.0, current release.** Requires React 18+.
...
export function ChatWithThreads() {
  ...
  const {
    ...
    agentId, // from useAgents or PersonaProvider defaultAgentId
  ...
  const handleNewChat = () => {
    startNewChat(); // instant, no POST until first send
...
When `threadId` is `undefined` (ephemeral new chat, 0.8.0+) the first `sendMessage` auto-mints a real thread via `POST /threads {agentId}` before streaming — no need to call `createThread()` yourself. `onThreadCreated` fires with the new id so you can sync `activeThreadId` and refetch the sidebar.
...
if (interrupt) {
  if (interrupt.kind === 'hitl') {
    ...
    interrupt.actionRequests.map((req) => (
      <div key{} className="border rounded p-3">
        <p>Agent wants to: <strong{}</strong></p>
        <pre{}</pre>
        <button onClick={() =>
          resumeInterrupt(
            { decisions: {}] },
...
const {
  ...
  sendMessage,        // (content?: string, override?: SendMessageOverride) => Promise<boolean>
  ...
  reload,             // () => Promise<boolean> — resend the last user message (0.8.0 fixed dup)
  clear,              // () => void — reset all state
  startNewChat,       // () => void — 0.8.0 ephemeral: clear and enter new-chat mode
  currentThreadId,    // string | undefined — effective thread id (prop ?? minted)
  isEphemeral,        // boolean — true when no real thread yet
