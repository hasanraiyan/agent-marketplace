...
| `TOOL_CALL_RESULT` | Sets `result` on the matching `PersonaToolCall`. Also detects `present_file` results and sets `presentedFile`. Detects error status via {}` JSON envelope. |
...
```tsx
const [mcpApps, setMcpApps] = useState<Map<string,{}>>(
  new Map()
);
...
TOOL_CALL_CHUNK    (id: "tc-1", name: "delete_file", delta: {}')
