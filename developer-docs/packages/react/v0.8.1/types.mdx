...
interface PersonaMessage {
  id: string;
  role: PersonaRole;            // 'reasoning' = one thinking phase (0.5.0+)
  content: string;
  createdAt: Date;
  isStreaming?: boolean;        // true while this message is being received
  toolCalls?: PersonaToolCall[];
  seq?: number;                 // monotonic stream order (reasoning + tool calls share counter, 0.5.1+)
  // deprecated (0.5.0): assistant reasoning now streams as separate role:'reasoning' messages
  reasoning?: string;
  isReasoning?: boolean;
}
```

`role: 'reasoning'` messages are one per thinking phase — inserted above the assistant message and interleaved with tool calls via `seq`. An assistant turn is `reasoning → tool → reasoning → answer`; `content` on the assistant message is the final answer only.
...
interface UseVoiceOptions {
  ...
  contextOverride?: string; // appended to the system instruction once, at connect (0.7.6+)
}
...
interface UseChatOptions {
  agentId?: string;
  threadId?: string; // undefined = ephemeral new chat (0.8.0)
  initialMessages?: PersonaMessage[];
  onFinish?: (message: PersonaMessage) => void;
  onError?: (error: Error) => void;
  onEvent?: (event: PersonaStreamingEvent) => void;
  onThreadCreated?: (threadId: string) => void; // 0.8.0: ephemeral → real mint
  context?: Record<string, unknown> | (() => Record<string, unknown>); // 0.7.8
  voice?: UseVoiceResult; // merges a useVoice() call's live transcript into `messages` — see below
}
```

`voice` (added in `0.7.4`): pass the object returned by [`useVoice()`](/guides/react/hooks#usevoice)
(sharing the same `threadId`) and `useChat` merges its live turns into `messages` for you — see
[Syncing voice into the chat feed](/guides/react/hooks#syncing-voice-into-the-chat-feed) for what
that gets you and what's still your own responsibility.

`onThreadCreated` (0.8.0): fired when an ephemeral chat auto-mints a real thread on first `sendMessage`. Host should `setThreadId(id)` / `refetch()`.

`context` (0.7.8): never shown to the model, only read live by an RCP tool. Object or getter, shallow-merged with per-send `context`.
...
interface SendMessageOverride {
  agentId?: string;
  threadId?: string | Promise<string | undefined>; // 0.3.4+: awaited right before POST
  resume?: PersonaResumeValue;
  context?: Record<string, unknown>; // 0.7.8+: per-send RCP context
  contextOverride?: string; // appended to the system prompt for this turn only (0.7.5+)
}
```
