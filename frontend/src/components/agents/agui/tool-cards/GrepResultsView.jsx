...
export function parseGrepResults(resultText) {
  ...
  if (matches.length === 0) {
    return lines
      .map((l) => l.trim())
      .filter(
        (l) => l && (l.startsWith("/") || l.includes(".") || l.includes("\\")),
      )
...
export function GrepResultsView({ tool, done }) {
  ...
  const query =
    parsedInput.pattern || parsedInput.Query || parsedInput.query || "";
  const path =
    parsedInput.path || parsedInput.SearchPath || parsedInput.searchPath || "/";
          ...
          <div className="max-h-60 overflow-auto space-y-2.5 scrollbar-thin pr-1">
            {groupEntries.map(([filePath, matches]) => {
              ...
              return (
                <div
                  key={filePath}
                  className="rounded-xl border border-slate-150 bg-white dark:border-slate-800/80 dark:bg-slate-950 overflow-hidden"
                >
                  {/* File group header */}
                  ...
                  <div className="divide-y divide-slate-50 dark:divide-slate-850 font-mono text-[11px] leading-relaxed">
                    {matches.map((match, i) => (
                      <div
                        key={i}
                        className="flex hover:bg-slate-50/50 dark:hover:bg-slate-900/30"
                      >
                        {/* Line number column */}
                        {match.line > 0 && (
