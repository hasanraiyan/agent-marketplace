  ...
  const handleTest = async () => {
    ...
    try {
      ...
      const msg =
        ...
        res.data?.message ||
        (tools ? `Found ${tools.length} tool(s).` : "Connection successful.");
      setTestResult(msg);
      if (tools)
        setSource((prev) =>
          prev
    ...
    <div className="flex w-full flex-col gap-4 overflow-y-auto p-4 sm:gap-6 sm:p-6 lg:p-8">
      ...
      <div className="flex flex-col gap-1">
        ...
        <p className="max-w-2xl text-xs leading-snug text-muted-foreground sm:text-sm">
          Update the manifest URL, auth, or enabled state.
        </p>
      </div>
      ...
      <div className="grid w-full items-start gap-4 sm:gap-6 lg:grid-cols-[1.65fr_0.85fr]">
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Test the connection after saving to discover the tools this source
              exposes.
            </CardDescription>
          </CardHeader>
          <CardContent>
                    ...
                    <Field>
                      ...
                      >
                        <SelectTrigger id="secretRef">
                          <SelectValue
                            placeholder={
                              secrets === null ? "Loading…" : "Select a secret"
                            }
                          />
                        </SelectTrigger>
                        <SelectContent>
        ...
        <div className="flex flex-col gap-4">
            ...
            <CardContent className="flex flex-col gap-0 divide-y divide-border text-xs">
              <div className="flex items-center justify-between gap-4 py-2.5">
                ...
                <span className="flex items-center gap-1.5">
                  <span className="max-w-[140px] truncate font-mono text-[11px] sm:max-w-[180px]">
                    {sourceIdDisplay}
                  </span>
                  <Button
                    variant="ghost"
            ...
            <Card>
              ...
              <CardContent className="flex flex-col gap-2.5 text-xs">
                {source.tools.map((t) => (
                  <div
                    key={t.name}
                    className="flex flex-col gap-1 rounded-none border border-border/60 bg-muted/20 p-2.5"
                  >
                      ...
                      <div className="flex flex-wrap gap-1 pt-1 border-t border-border/40">
                        {t.params.map((p) => {
                          ...
                          return (
                            <span
                              key={p.name}
                              className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-[10px] font-mono border ${
                                isMapped
          ...
          {allParams.length > 0 && (
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Map tool parameters to runtime context keys. Mapped parameters
                  are resolved live and hidden from the LLM.
                </CardDescription>
              </CardHeader>
              <CardContent>
                <ContextParamMapper
                ...
                isUsed ? (
                  <div className="flex flex-col gap-2">
                    <p className="text-muted-foreground">
                      Used by {usage.agentCount} agent(s):
                    </p>
          ...
          {isUsed && usage && (
            <Alert>
              ...
              <AlertDescription>
                {usage.agents.map((a) => a.name).join(", ")}. Update those
                agents before deleting.
              </AlertDescription>
            </Alert>
          )}
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              Delete RCP source "{source.name}"?
            </AlertDialogTitle>
            <AlertDialogDescription>
              This cannot be undone.{" "}
              {isUsed
                ? `This source is still used by ${usage?.agentCount} agent(s). Update those agents first.`
                : "Agents using this source will lose access to its tools."}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
