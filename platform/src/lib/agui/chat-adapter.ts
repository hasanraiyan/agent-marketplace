 ...
 * - `reasoning`/`block` messages are dropped from display; while a run is
 *   live and a turn has produced only reasoning so far, the row stays an
 *   empty streaming assistant row so `ChatMessage` shows its ThinkingIndicator.
...
export function toChatView(input: ChatAdapterInput): ChatView {
  ...
  for (const entry of conversation) {
    if (entry.type === "message") {
      ...
      // reasoning / block messages are intentionally not shown.
...
export function hitlInterruptFrom(
  approval: { actionRequests: Array<{ type: string; label: string }> },
): ChatInterruptData {
  const actionRequests: ChatHitlAction[] = approval.actionRequests.map(
    (action, index) => ({
      ...
      // index is the only stable handle back to the request. Using String(index)
      // also avoids colliding when two actions share a type value.
      id: String(index),
