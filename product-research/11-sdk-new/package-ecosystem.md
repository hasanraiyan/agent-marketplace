...
| `@personaai/runtime` | `sdk/runtime/` | **v0.9.5 — Shipped** | Framework-agnostic runtime engine. |
...
| `@personaai/adapters` | `sdk/adapters/` | **v0.1.0 — Shipped** | Unified adapter — `express`/`nextjs`/`nestjs` subpaths, shared `src/shared/` core. Single dep. |
| `@personaai/express` | `sdk/adapters/express/` | **Deprecated shim → `@personaai/adapters/express`** | Frozen re-export, will be removed in 1.0. |
| `@personaai/nestjs` | `sdk/adapters/nestjs/` | **Deprecated shim → `@personaai/adapters/nestjs`** | Frozen re-export, will be removed in 1.0. |
| `@personaai/nextjs` | `sdk/adapters/nextjs/` | **Deprecated shim → `@personaai/adapters/nextjs`** | Frozen re-export, will be removed in 1.0. |
...
graph TD
    subgraph "What developers install"
        ADAPTERS["@personaai/adapters<br/>express / nextjs / nestjs<br/>(single dep, shared core)"]
    ...
    style ADAPTERS fill:#0f3460,stroke:#e94560,color:#fff
...
### 3. `@personaai/adapters` — Unified Adapter (Hero Package)

> ✅ **Shipped** as `@personaai/adapters` v0.1.0 — single dep, shared `src/shared/` core, `express`/`nextjs`/`nestjs` via subpaths. Legacy names `@personaai/nextjs`/`@personaai/express`/`@personaai/nestjs` are deprecated shims re-exporting from it.

| | |
|---|---|
| **Audience** | Next.js (primary), Express, NestJS developers — one install |
| **Environment** | Server-side (route handlers / Router / Module) + client re-export for Next.js |
| **Depends on** | `@personaai/runtime`, `@personaai/react` (for `nextjs` client), `@personaai/logger` |
| **Used by** | Developers directly |
| **What it does** | **Single package, three frameworks:** `import { createPersonaHandler } from '@personaai/adapters/nextjs/server'` / `import { toExpressRouter } from '@personaai/adapters/express'` / `import { PersonaModule } from '@personaai/adapters/nestjs'`. Each is a thin translation layer sharing `src/shared/` (errors, headers, multipart, write, translate, handler-node). Fix once, ship everywhere. Client side re-exports `@personaai/react` so Next.js devs still install one package. |
| **Why unified** | `express`/`nestjs` were 95% copy-paste, `nextjs` only diverged for Web `Request`/`ReadableStream`. One publish, one version, no `file:` inter-package links, no drift. Legacy packages remain as frozen shims for back-compat. |
| **Size** | Very small per subpath — just the adapter glue. All logic lives in `@personaai/runtime` + `src/shared/`. |

**What the Next.js developer does:**
1. `npm install @personaai/adapters` (peer: `next`, `react`)
2. Creates one catch-all route file `app/api/persona/[...persona]/route.ts` that exports the handler from `@personaai/adapters/nextjs/server`
3. Wraps layout in `PersonaProvider` from `@personaai/adapters/nextjs`
4. Uses hooks

**What the Express developer does:**
1. `npm install @personaai/adapters` (peer: `express`)
2. `import { createExpressAdapter } from '@personaai/adapters/express'` → `app.use('/api/persona', persona.router)`

**That's it — one package, no second install for the other framework.**

<details><summary>Legacy: `@personaai/nextjs` / `@personaai/express` / `@personaai/nestjs` (deprecated shims)</summary>

`@personaai/nextjs` was the hero package, `@personaai/express` was the Express adapter. Both shipped as separate packages and are now deprecated shims re-exporting from `@personaai/adapters`. New code must import from `@personaai/adapters/*`.
