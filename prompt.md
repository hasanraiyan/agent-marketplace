...
`@personaai/runtime` (`runtime/`) is a framework-agnostic runtime engine for the Persona
Developer Platform — the shared engine every future framework adapter (`@personaai/express`,
`@personaai/nextjs`, ...) is meant to wrap. It sits on top of `@personaai/sdk` (`sdk/`) and turns
...
It was built from scratch across v0.1 → v0.5 in a prior session (see `runtime/README.md` for the
...
1. **`sdk/` got a real bug fix and a version bump**, published to npm as `@personaai/sdk@0.4.1`:
   - Removed an accidental self-dependency in `sdk/package.json` (`"@personaai/sdk": "^0.3.1"`
     listed as its own dependency — copy-paste bleed, almost certainly from `runtime/package.json`).
   ...
   - Verify: `cd sdk && npm view @personaai/sdk version` should print `0.4.1`, and
...
### Pass 1 — `runtime/` package correctness
...
Also sanity-check `runtime/package.json`'s `dependencies` — it should list `@ag-ui/core` and
`@personaai/sdk` (pinned `^0.4.0` or looser), and **must not** list itself as its own dependency
(the exact bug just fixed in `sdk/package.json` — check runtime doesn't have the same mistake).
...
- **`quickstart.mdx`** — `createRuntime()`'s option table (types, defaults, required/optional)
  against `runtime/src/types/options.ts`'s `CreateRuntimeOptions`. The `RuntimeRequest`/
  `RuntimeResponse` shapes against `runtime/src/types/request.ts` and `response.ts`.
- **`routes.mdx`** — every row of both tables (always-on and opt-in) against
  `runtime/src/runtime.ts`'s `buildRoutes()` and the individual `runtime/src/routes/*.ts` files.
  Check no route was added/removed/renamed since this doc was written, and that the capability
  column matches `runtime/src/types/options.ts`'s `RuntimeCapabilities`.
- **`capabilities.mdx`** — the default-off claim and the two-mount pattern against
  `runtime/src/runtime.ts`'s `resolveCapabilities()`.
- **`hooks.mdx`** — all eight hooks' signatures and context shapes against
  `runtime/src/types/hooks.ts`.
- **`reconnect.mdx`** — the `RunDriver`/eviction/heartbeat/backpressure claims against
  `runtime/src/runDriver.ts`, `runtime/src/runRegistry.ts`, and `runtime/src/heartbeat.ts`. Pay
...
- **`errors.mdx`** — the error shape and two-tier trust model against `runtime/src/errors.ts`.
...
- **Consistency with `runtime/README.md`** — the docs were adapted from the README's content but
...
- **Pass 1**: pass/fail on each command above, and — critically — whether `runtime/package.json`
