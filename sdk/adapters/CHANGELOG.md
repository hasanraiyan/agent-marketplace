...
- The `VERSION` constant exported from `express`, `nestjs`, `nextjs/client`, and `nextjs/server`
  was hardcoded to `'0.3.0'` in all four — never updated across four releases since. Now `'0.7.1'`
  in all four.

### Changed

- Docs only: top-of-file version note in README updated from a stale `v0.1.0`.
