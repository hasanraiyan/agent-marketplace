...
/**
 * Writes a runtime response onto a Node/Express/Nest response.
 * Unified for express + nestjs: handles `res.set` vs `res.setHeader`,
 * `res.flushHeaders` / `res.flush` optional, backpressure via `drain`.
 */
export async function writeRuntimeResponse(
  ...
  // Nest/Express streaming helpers: X-Accel-Buffering for nginx, flushHeaders for SSE
  if (typeof res.setHeader === 'function' && kind === 'stream') {
    try {
      // For nginx buffering off; harmless if already set
      if (!response.headers['x-accel-buffering'] && !response.headers['X-Accel-Buffering']) {
