...
describe('ThreadsResource', () => {
  ...
  it('listFiles() GETs the files sub-route and returns the files map', async () => {
    const files = { '/notes.md': { content: 'hi', size: 2, created_at: null, modified_at: null } };
    const fetchMock = vi.fn(async (_url: string, _init?: RequestInit) =>
      jsonResponse({ success: true, data: files })
    );
    const client = makeClient(fetchMock as unknown as typeof fetch);

    const result = await client.threads.listFiles('t1');
    expect(result).toEqual(files);
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('https://api.example.com/api/v1/developer/threads/t1/files');
    expect(init.method).toBe('GET');
  });

  it('getFile() GETs the file sub-route with a path query param', async () => {
    const file = { content: 'hi', size: 2, created_at: null, modified_at: null };
    const fetchMock = vi.fn(async (_url: string, _init?: RequestInit) =>
      jsonResponse({ success: true, data: file })
    );
    const client = makeClient(fetchMock as unknown as typeof fetch);

    const result = await client.threads.getFile('t1', '/notes.md');
    expect(result).toEqual(file);
    const [url] = fetchMock.mock.calls[0] as [string];
    expect(url).toBe('https://api.example.com/api/v1/developer/threads/t1/file?path=%2Fnotes.md');
  });

  it('writeFile() PUTs { path, content } to the file sub-route', async () => {
    const file = { content: 'hello', size: 5, created_at: 't0', modified_at: 't1' };
    const fetchMock = vi.fn(async (_url: string, _init?: RequestInit) =>
      jsonResponse({ success: true, data: file })
    );
    const client = makeClient(fetchMock as unknown as typeof fetch);

    const result = await client.threads.writeFile('t1', '/notes.md', 'hello');
    expect(result).toEqual(file);
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('https://api.example.com/api/v1/developer/threads/t1/file');
    expect(init.method).toBe('PUT');
    expect(JSON.parse(init.body as string)).toEqual({ path: '/notes.md', content: 'hello' });
  });

  it('deleteFile() DELETEs the file sub-route with a path query param', async () => {
    const fetchMock = vi.fn(async (_url: string, _init?: RequestInit) =>
      jsonResponse({ success: true, message: 'Workspace file deleted successfully' })
    );
    const client = makeClient(fetchMock as unknown as typeof fetch);

    await client.threads.deleteFile('t1', '/notes.md');
    const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit];
    expect(url).toBe('https://api.example.com/api/v1/developer/threads/t1/file?path=%2Fnotes.md');
