...
| 2 (revision) → clients round 3 | Dev came back with four fixes: use numbers already given, two-question intake instead of a form, score table inline in chat, PCI → security hand-off. ~14 exchanges; the simulated creator over-specified ("hard gate", "numeric header") before the simulator was made more realistic. | 21 / 20 · both "would return" | Rohit still got the intake card re-asked once after supplying numbers (pacing note). Tanvi: nothing structural; the judge wanted the client made to articulate rejected alternatives herself. | Nothing further needed for the bar; noted the intake re-ask for the next revision. |
