...
|---|---|---|---|---|
| 1 | 46 turns (long: the creator kept refining). Persona with tagline + category. One skill, published with title/hook/category and attached, 7 topic files. The Architect used `test_persona` and showed real replies; the creator gave one correction each time. | 19 / (run killed by OS before Rohit was judged; rerun pending) | Persona still named with the slug (upstream `createAgent` overwrote the name with the username — fixed in code). Intake section absorbed the URL-shortener example's specifics ("total active links", "TTL") and grew into a ten-field form. Judge docked usefulness because the harness didn't show presented files (fixed in harness). | `createAgent` keeps the creator's display name. Skill-training rule: intake is generic to the verb+object and conversational; example specifics stay in section 7. Harness surfaces presented files to the client and judge. |
